 ...
 * @LastEditors: mingwei
 * @LastEditTime: 2022-06-17 13:40:31
...
const copyTemplate = answers => {
  ...
  spinner.start('初始化中...');
  const originalUrl = path.resolve(__dirname, `../block/${blockTemplate}`);
  const targetUrl = path.resolve(cwd);

  // 拷贝文件
  shell.cp('-R', path.resolve(cwd, originalUrl), path.resolve(cwd, targetUrl));
  // copyFile(
  //   path.resolve(__dirname, `../block/${blockTemplate}`),
  //   path.resolve(__dirname, `../${blockName}`),
  // );
  spinner.succeed(chalk.bgGreen('创建区块成功'));
};
